...
const TimelineSection = ({
  ...
  useEffect(() => {
    const fetchTimeline = async () => {
      try {
        const response = await fetch('/admin/api/data/timeline');
        ...
        // Apply slicing if maxEvents is prop provided
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-7xl mx-auto relative">
          ...
          {events.length > 0 ? events.map((event, index) => {
            ...
            return (
              <div key={event.id || index} className="relative mb-16 hidden md:block">
            ...
            <div className="text-center text-gray-400 py-10 hidden md:block">Loading timeline...</div>
          )}

          {/* Mobile layout */}
          {events.map((event, index) => {
            return (
              <div key={`mobile-${event.id || index}`} className="relative mb-6 md:hidden">
